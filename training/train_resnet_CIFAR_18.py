...
def train_and_evaluate(epochs=30, num_classes=10):
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_classes):
            row[f"Class{i}_Acc"] = correct[i] / total[i] if total[i] > 0 else 0.0
            row[f"Class{i}_Prob"] = prob_sums[i] / total[i] if total[i] > 0 else 0.0
